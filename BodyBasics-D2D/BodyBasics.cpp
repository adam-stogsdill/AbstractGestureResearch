                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
							...
							float left_hand_X = BodyToScreen(joints[JointType_HandLeft].Position, width, height).x;
							float left_hand_Y = BodyToScreen(joints[JointType_HandLeft].Position, width, height).y;

							std::ofstream stream;
							stream.open("C:/Users/Adam's PC/Desktop/SkeletonBasics-D2D/TextFile1.txt", std::fstream::app);
							stream << buffer << left_hand_X << "," << left_hand_Y << "," << joints[JointType_HandLeft].Position.Z << ")" << std::endl;
