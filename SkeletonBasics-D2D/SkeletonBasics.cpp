...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	time(&raw_start);
	timeinfo = localtime(&raw_start);
	start_time = asctime(timeinfo);
...
{      
	...
	time_t now;
	time(&now);
	strftime(buffer, 80, "%I:%M:%S,", localtime(&now));
	puts(buffer);


	D2D1_POINT_2F HAND_LEFT_COORDINATES = SkeletonToScreen(skel.SkeletonPositions[NUI_SKELETON_POSITION_HAND_LEFT], windowWidth, windowHeight);
	
	/*
	File Format
	TIME_STAMP, HAND_LEFT_X, HAND_LEFT_Y, HAND_LEFT_Z, HAND_RIGHT_X, HAND_RIGHT_Y, HAND_RIGHT_Z, HEAD_X, HEAD_Y, HEAD_Z
	
	*/

	std::ofstream stream;
	stream.open("C:/Users/Adam's PC/Desktop/SkeletonBasics-D2D/TextFile1.txt", std::fstream::app);
	stream << buffer << HAND_LEFT_COORDINATES.x << "," << HAND_LEFT_COORDINATES.y << "," << skel.SkeletonPositions[NUI_SKELETON_POSITION_HAND_LEFT].z << ")" << std::endl;
